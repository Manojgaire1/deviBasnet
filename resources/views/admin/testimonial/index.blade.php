...
@section('page_title','Testimonial lists')
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Add New Testimonails</h5>
                     ...
                     <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="name" id="name" placeholder="Yam N. Timsina(PhD.)">
                            </div>
                            <div class="form-group">
                                <label for="position">Company Name</label>
                                <input type="text" class="form-control" name="company_name" id="company_name" placeholder="Inkwalkar Pvt. Ltd.">
                            </div>
                            <div class="form-group">
                                <label for="position">Position</label>
                                <input type="text" class="form-control" name="position" id="position" placeholder="Manager">
                            </div>
                            <div class="form-group">
                                <label>Status</label>
                            ...
                            <div class="form-group">
                                ...
                                <div class="file-upload">
                                    <div class="image-upload-wrap"  style="background: url({{asset('/admin-assets/images/user-profile/2012_Councelling_Roka_2.png')}});background-size: contain;">
                                        ...
                                        <input class="file-upload-input" type="file" name="testimonial_image_path" onchange="previewFile(this);" accept="image/*">
                                        <div class="drag-text">
                                            <div class="icon">+</div>
                                        </div>
                                    </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootbox.js/4.4.0/bootbox.min.js"></script>
    ...
    <script>
        $(document).ready(function () {
            ...
            testimonialTable = $('.activity_table').DataTable({
                dom: 'Blfrtip',
                buttons: [],
                order: [[0,'desc']],
                autoWidth: false,
            ...
            form,
            {
                ...
                testimonialId = $("#testimonial_id").val();
                if (save_method == 'update') {
                    testimonial_url = "{{route('admin.testimonial.update',':make')}}";
                    testimonial_url = testimonial_url.replace(":make",testimonialId);
                } else {
                    testimonial_url = "{{route('admin.testimonial.store')}}";
                }
                ...
                $.ajax({
                    ...
                    error: function (jqXHR,textStatus,errorThrown) {
                        if(jqXHR.status == 500){
                            console.log('There is server error adding the new testimonial, please try again');
                        }
                    }
            ...
            $('body').on('click', '.add-new-make', function () {
                save_method = "add";
                $("h5.modal-title").html('Create new testimonial')
                $("#testimonial_id").val('');
                initTinyMce();
            ...
            $('body').on('click','.edit-btn',function(e){
                ...
                testimonialId = $(this).data('testimonial-id');
                $("#testimonial_id").val(testimonialId);
                testimonial_url   = "{{url('/admin/testimonials/')}}" + "/" + testimonialId + "/edit";
                $.get(testimonial_url ,function(response){
                    ...
                    if(response.status == "success"){
                        ...
                        swal('Not found!!', 'Testimoinal not found in the server', 'error');
                    }
                });
            });
            ...
            $("body").on('click','.delete-btn', function(e){
                e.preventDefault();
                testimonialId=$(this).attr('data-testimonial-id');
                testimonial_url  = "{{url('/admin/testimonials/')}}" + "/" + testimonialId;
                swal({
                    title: "Are you sure?",
                    text: "You will not be able to recover testimonial!",
                    icon: "warning",
                    buttons: {
                        cancel: "Cancel",
                        catch: {
        ...
        function populateModalData(response,has_set_menu_image){
            ...
            $("h5.modal-title").html('Update ' + response.testimonial.client_name + ' testimonial')
            //populate the individual field
            $("#name").val(response.testimonial.client_name);
            $("#company_name").val(response.testimonial.company_name);
            $("#position").val(response.testimonial.position);
            //loop through each 
            var testimonial_status = $("select[name='status'] > option")
            testimonial_status.each((index,value) =>{
                if(response.testimonial.status == value.value){
                    $("select[name='status']").val(response.testimonial.status)
                }
            })

            //set the content in the tinymce
            if(response.testimonial.description != null)
                tinymce.get('description').setContent(response.testimonial.description);
            //set the if provided
            if(response.testimonial.featured_image != null){
                var image_directory = "{{asset('storage/uploads/testimonials/large')}}";
                $(".image-preview-single").attr('src',image_directory + "/" + response.testimonial.featured_image);
            }
        }
