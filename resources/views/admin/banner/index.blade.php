...
@section('page_title','Banner lists')
                    ...
                    <div class="card card-white">
                        ...
                        <div class="card-block">
                            <div class="table-responsive dt-responsive">
                                <table class="table table-striped table-bordered data-table banner_table">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>Image</th>
    ...
    <div class="modal-dialog modal-" role="document">
        <div class="modal-content">
            ...
            <form id="bannerForm" name="bannerForm">
                @csrf
                <div class="modal-body banner_add_body">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                ...
                                <div class="file-upload">
                                    <div class="image-upload-wrap"
                                        ...
                                        <input class="file-upload-input" type="file" name="banner_image_path"
...
<script>
    $(document).ready(function () {
            ...
            makeTable = $('.banner_table').DataTable({
                dom: 'Blfrtip',
                buttons: [],
                order: [[0,'desc']],
                autoWidth: false,
            ...
            form,
            {
                ...
                makeId = $("#banner_id").val();
                if (save_method == 'update') {
                    make_url = "{{route('admin.banner.update',':make')}}";
                    make_url = make_url.replace(":make",makeId);
                } else {
                    make_url = "{{route('admin.banner.store')}}";
                }
                // get the input values
                result = new FormData($("#bannerForm")[0]);
                $.ajax({
                    url: make_url,
                    data:result,
                    dataType: "Json",
            ...
            $('body').on('click', '.add-new-make', function () {
                save_method = "add";
                $("h5.modal-title").html('Create new banner')
                $("#banner_id").val('');
                initTinyMce();
            ...
            $('body').on('click','.edit-btn',function(e){
                ...
                makeId = $(this).data('banner-id');
                $("#banner_id").val(makeId);
                menu_url   = "{{url('/admin/banners')}}" + "/" + makeId + "/edit";
            ...
            $("body").on('click','.delete-btn', function(e){
                e.preventDefault();
                makeId=$(this).attr('data-banner-id');
                make_url  = "{{url('/admin/banners')}}" + "/" + makeId;
                swal({
                    title: "Are you sure?",
        ...
        function populateModalData(response,has_set_menu_image){
            ...
            $("h5.modal-title").html('Update ' + response.banner.title + ' banner')
            //populate the individual field
            $("#title").val(response.banner.title);
            //set the if provided
            if(response.banner.featured_image != null){
                var image_directory = "{{asset('storage/uploads/banners/large')}}";
                $(".image-preview-single").attr('src',image_directory + "/" + response.banner.featured_image);
            }
        }
